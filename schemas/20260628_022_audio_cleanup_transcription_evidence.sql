-- DnD Scribe - let cleanup recognize persisted transcription evidence for intermediate audio.
...
with compact_tracks as (
  ...
  left join audio_speech_slices ss
   ...
   and ss.transcription_status in ('transcribed', 'cached')
  left join audio_chunks ac
   ...
   and ac.transcription_status in ('transcribed', 'cached')
...
set
  ...
  metadata = coalesce(aa.metadata, '{}'::jsonb) || jsonb_build_object(
    'marked_delete_ready_by', '20260628_022_audio_cleanup_transcription_evidence',
...
select
  ...
  'marked_delete_ready',
  'Marked delete_ready by transcription evidence policy; no R2 object was deleted.',
  jsonb_build_object(
    'migration', '20260628_022_audio_cleanup_transcription_evidence',
...
where candidate.readiness_status = 'delete_ready'
  and not exists (
    ...
    where event.artifact_id = candidate.artifact_id
      ...
      and event.payload->>'migration' = '20260628_022_audio_cleanup_transcription_evidence'
  );
