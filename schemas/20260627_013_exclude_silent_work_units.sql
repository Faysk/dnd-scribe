-- DnD Scribe - exclude silent audio from paid transcription work units.
...
where coalesce(ss.probably_silent, false) is false
union all
select
...
where coalesce(ac.probably_silent, false) is false
  and not exists (
    select 1
    from audio_speech_slices ss
