-- DnD Scribe - treat skipped silent speech slices as cleanup evidence.

update audio_speech_slices
set
  transcription_status = 'skipped_silence',
  metadata = coalesce(metadata, '{}'::jsonb) || jsonb_build_object(
    'marked_skipped_silence_by', '20260628_023_silent_speech_slice_cleanup',
    'marked_skipped_silence_at', now()
  ),
  updated_at = now()
where probably_silent is true
...
set
  ...
  metadata = coalesce(aa.metadata, '{}'::jsonb) || jsonb_build_object(
    'marked_delete_ready_by', '20260628_023_silent_speech_slice_cleanup',
...
select
  ...
  'marked_delete_ready',
  'Marked delete_ready by skipped silence cleanup policy; no R2 object was deleted.',
  jsonb_build_object(
    'migration', '20260628_023_silent_speech_slice_cleanup',
    'readiness_status', candidate.readiness_status,
    'artifact_type', candidate.artifact_type,
    'reclaimable_bytes', candidate.reclaimable_bytes,
    'transcription_evidence_objects', candidate.transcription_evidence_objects
  )
...
where candidate.readiness_status = 'delete_ready'
  and not exists (
    ...
    where event.artifact_id = candidate.artifact_id
      ...
      and event.payload->>'migration' = '20260628_023_silent_speech_slice_cleanup'
  );
