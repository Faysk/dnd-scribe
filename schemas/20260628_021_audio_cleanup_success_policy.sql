-- DnD Scribe - let delete_after_success artifacts become ready as soon as success evidence exists.
...
set
  ...
  metadata = coalesce(aa.metadata, '{}'::jsonb) || jsonb_build_object(
    'marked_delete_ready_by', '20260628_021_audio_cleanup_success_policy',
...
select
  ...
  'marked_delete_ready',
  'Marked delete_ready by cleanup success policy; no R2 object was deleted.',
  jsonb_build_object(
    'migration', '20260628_021_audio_cleanup_success_policy',
...
where candidate.readiness_status = 'delete_ready'
  and not exists (
    ...
    where event.artifact_id = candidate.artifact_id
      ...
      and event.payload->>'migration' = '20260628_021_audio_cleanup_success_policy'
  );
