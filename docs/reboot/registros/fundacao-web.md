...
O proprietário autorizou alterações diretamente em main para iniciar o reboot. Exigiu Node 26 em produção mesmo que isso implique mudar o hosting, e determinou serviços gratuitos. Um runner local no WSL pode ser usado se necessário. Nenhum plano pago, recurso externo novo ou mudança de DNS faz parte desta entrega.
...
- Node 26.8.1, pnpm 12.3.4 e dependências diretas web fixadas nas últimas versões verificadas. [Matriz](versoes.md).
...
- CI em Node 26 para main/Preview, incluindo compilação e smoke do contêiner em produção. A suíte Playwright anterior continua em modo dev; o smoke de produção é uma verificação adicional, não uma alegação de E2E completo do novo hosting.
...
Os arquivos Vercel desativam o deploy por Git da nova base incompatível com Node 24. Deployments já existentes não são removidos. Antes de mudar DNS: validar origem gratuita e limites, configurar metadata/env/OAuth, testar navegação e login no ambiente candidato, reconciliar dados e só então promover o domínio. A base não está publicada no domínio só porque foi enviada a main.
