...
async function uploadObject(storage, objectPath, bytes, cacheControl) {
...
async function verifyImmutableObject(storage, objectPath, sha256) {
...
async function handleCompanionRelease(req) {
  ...
  // A build can change bytes without changing the semantic Companion version. Keying
  // the archival object by SHA makes verification immune to CDN caching from an
  // earlier 0.4.0 build and gives every published binary an immutable identity.
  const immutableObject = `windows/${version}/${actualSha}/DnDScribeCompanionSetup.exe`;
  await uploadObject(storage, immutableObject, bytes, '31536000');
  await verifyImmutableObject(storage, immutableObject, actualSha);

  // Keep the legacy alias because the authorized download endpoint already signs
  // this path. A zero cache lifetime minimizes stale bytes after an in-place release.
  await uploadObject(storage, LEGACY_OBJECT, bytes, '0');
