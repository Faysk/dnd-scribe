...
export function PendingAccess({ identity, returnHref = '/sessoes' }: PendingAccessProps) {
  return (
    <div className="mx-auto grid min-h-[calc(100vh-77px)] w-[min(1180px,calc(100%-2.5rem))] place-items-center py-12">
      <Surface className="w-full max-w-2xl p-7 sm:p-10 lg:p-14" tone="elevated">
        <Eyebrow>Acesso interno pendente</Eyebrow>
        <DisplayTitle className="mt-4">A memória continua aberta.</DisplayTitle>
        <BodyCopy className="mt-6 max-w-xl">
          Seu login está conectado e os resumos públicos continuam disponíveis. Transcrições, downloads e material bruto só aparecem depois que seu perfil for aprovado como membro da campanha.
        </BodyCopy>
        <MetaText className="mt-5">Conectado como {identity.displayName}</MetaText>
        <div className="mt-8 flex flex-wrap gap-3">
          <ActionLink href={returnHref} variant="secondary">Voltar à memória pública</ActionLink>
